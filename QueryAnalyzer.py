...
class AnnotatedQuery:
    ...
    def __init__(self, query, shows, theaters, people, genres):
        ...
        self.keywords = {'keywords': {'NOUN': [], 'VERB': []},
                         'edges': {'ROOT': [], 'NSUBJ': [], 'POBJ': [], 'ATTR': []}}
        ...
        for s in shows:
            if re.search(s, query) is not None:
                self.shows.append(s)

        for t in theaters:
            if re.search(t, query) is not None:
                self.theaters.append(t)

        for p in people:
            if re.search(p, query) is not None:
                self.people.append(p)

        for g in genres:
            if re.search(g, query) is not None:
